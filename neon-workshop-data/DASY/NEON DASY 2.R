...
library(tidycensus)
census_api_key("90b94953d2f24e81e890229e0128174f5ba80d3f", overwrite = TRUE)
library(FedData)
library(tidyverse)
library(raster)
...
imp.surf.desc <- raster("/research-home/rswanwick/NLCD_2016_Impervious_descriptor_L48_20190405.img") #will want this path name to be relative/right now it's my local downloads file
...
#break the previous imp.surf.crop line into sections to try and see where we are getting an error 
Pop.projected_sp_prj <- spTransform(as(pop.projected, "Spatial"), CRSobj = proj4string(imp.surf.desc))
imp.surf.crop <- raster::crop(imp.surf.desc, Pop.projected_sp_prj)
...
Get_Dataset <- function(stid, ctyid){
  
  pop <- get_acs(geography = "block group", , variables = acs_vars, 
                 year = 2016, state= stid, county = ctyid, 
                 geometry = TRUE)   
  
  lu <- get_nlcd(template = pop, label = paste0(stid, ctyid),year = 2016, dataset = "Impervious")
  
  zero.pop <- get_decennial(geography = "block", variables = "dec_vars", 
                            year = 2010, state = stid, county = ctyid, 
                            geometry = TRUE) %>% filter(value == 0) %>% st_transform(., proj4string(lu))
...
#to use the function after you've created it Get_Dataset(acs_vars = “0008”, states = “MD”-- can leave as is or put a default value 
Get_Dataset <- function(acs_vars, states, ...){
  
#everything inside the curly brackets is the recipe for whats happening when run the function   
