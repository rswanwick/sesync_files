...
#not being able to work on this code because getting an error message on this 
...
imp.surf.crop <- raster::crop(imp.surf.desc, spTransform(as(pop.projected, "Spatial"), CRSobj = proj4string(imp.surf.desc)))
#plot(imp.surf.crop)
#hist(imp.surf.desc)
...
#Mask out roads (i.e, all NonNA values in imp.roadss.p)
...
#test to make sure that the summarized raster is roughly equivalent to the original estimates
