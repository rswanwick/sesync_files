...
site_centroids <- read_csv("/research-home/rswanwick/neon-workshop-data/site_centroids.csv")

neon_sites <- readr::read_csv("/research-home/rswanwick/neon-workshop-data/field-sitesNEON.csv")
...
save_bigmap <- function(site_no, my_vwidth = 12000, my_vheight = 8000){
  ...
  mp <- leaflet(neon_sites_sf) %>%
    ...
    addPolygons(data = aops_sf, weight = 10, 
